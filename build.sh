...
# Copyright (C) 2026 TestaMic (crDroid bringup)

# Main Variables
KERNEL_NAME="crDroid"    # Kernel's name
KERNEL_VER="4.14.357"     # Kernel's version
BUILD_HOST="testamic-buildserver"     # Build host name
EXTRA_NOTES="RESS-nomount"    # Extra notes to add to the zip name
...
DO_A72Q=1 # Use Galaxy A72 defconfig? 1 - yes, 0 - no
...
if [[ "$DO_A52Q" -eq 1 && "$DO_A72Q" -eq 1 ]]; then
    echo "ERROR: Please select only one device (A52 or A72)"
    exit 1
fi
